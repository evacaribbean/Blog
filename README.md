...
...only close down a Derby database if it's running in embedded mode. While an embedded database is an integrated part of an application (with only one user at a time). Unlike an application running in a network...
...
> Since JDBC 4.0 (which is included in Java SE 6 and forward) there's no need to register the driver, as JDBC driver manager detects and loads the driver [automatically](https://docs.oracle.com/javase/8/docs/api/java/sql/package-summary.html).   
...
And that's it. 🐠 Next blog [post](https://github.com/evacaribbean/Blog#different-database-connections) 🐳 will walk through on how to connect to a MySQL db and on how to use an embedded driver.
